...
for (i in 1:seq_tim) {
  ...
  res = foreach (num=1:num_sim, .combine = 'cbind', .multicombine = TRUE, .options.snow = opts,.export=c("mvrnorm","sym_threshold","kmeans_inference_1f")) %dopar% {
    ...
    # X_R<-matrix(rnbinom(n = n*(p-p1), size = r3, prob = prob3), nrow = n, ncol = p-p1)
    # X<-cbind(rbind(X_LU,X_LD),X_R)
    lis<-IVS_cluster(X,K=K)
    res_IVS<-sym_threshold(lis$psi,alpha,"1")$disc
    res_en<-sym_threshold(lis$bpsi,alpha,"1")$disc
    ...
    res_CAD<-rep(T,length(res_en))
    naive_p_1<-2*(1-pt(abs(sta(X)),df=n-2))
    naive_p_2<-2*(1-pt(abs(naive_split(X)),df=n/2-2))
    res_nai_1<-which(p.adjust(naive_p_1,method = "BH")<alpha)
    res_nai_2<-which(p.adjust(naive_p_2,method = "BH")<alpha)
    # naive_p<-p_CAD$naive_p
    #######cluster DE#######
    syn_X<-syn_MVNM(X)
    C=log10(2*(1-pt(abs(sta(syn_X)),df=n-2))+runif(p,0,0.00001))-log10(2*(1-pt(abs(sta(X)),df=n-2))+runif(p,0,0.00001))
    res_CD<-sym_threshold(C,alpha,"1")$disc
    #######norm_split########
    p_countsplit<-2*(1-pt(abs(norm_split(X)),df=n-2))
    res_countsplit<-which(p.adjust(p_countsplit,method = "BH")<alpha)
    return(list(PSI =res_IVS,PSI_en=res_en,count_split=res_countsplit,DS=res_DS,MDS=res_MDS,CAD=res_CAD,Cluster_DE=res_CD,naive_1=res_nai_1,naive_2=res_nai_2))
  } 
  stopCluster(cl)
